...
export class ModuleManager implements mykoop.ModuleManager {
  ...
  canModuleRequire(requiringModuleName: string, requiredModuleName: string) {
    var mykoopModule = this.modules[requiredModuleName];
    return mykoopModule &&
      mykoopModule.isCore ||
      mykoopModule.dependants[requiringModuleName];
  }
  ...
  initializeLoadedModules() : void {
    ...
    _.forEach(this.modules, function (currentModule) {
      ...
      if (currentModule.instance.setModuleManager) {
        currentModule.instance.setModuleManager(
          utils.__DEV__ ?
            // use proxy only in dev
            new ModuleManagerProxy(self, currentModule)
            : self
        );
      }
    });
...
class ModuleManagerProxy implements mykoop.ModuleManager {
  get: (moduleName: string) => mykoop.IModule;
  getMetaData: (callback: mykoop.ModuleMetaDataCallback) => void;

  constructor(moduleManager: ModuleManager, mykoopModule: Module) {
    this.getMetaData = _.bind(moduleManager.getMetaData, moduleManager);
    this.get = function(moduleName: string) {
      if(!moduleManager.canModuleRequire(mykoopModule.name, moduleName)) {
        mykoopModule.logger.warn(
          "Module [%s] requiring module [%s] not in his dependency list",
          mykoopModule.name,
          moduleName
        );
      }
      return moduleManager.get(moduleName);
    }
  }
}
